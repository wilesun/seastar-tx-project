 ...
 * 分布式事务管理器
 */
public abstract class AbstractDistributedPlatformTransactionManager
        implements PlatformTransactionManager, DistributedTransactionManagerDelegate, Serializable {
    ...
    public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition) throws TransactionException {
        ...
        if (!traceContext.hasTrace()) {
            TraceDefinition traceDefinition = TraceUtils.newTrace();
            traceContext.setTrace(traceDefinition);
        }
    ...
    protected DefaultTransactionStatus newTransactionStatus(
        ...
        SpanDefinition span = SpanUtils.newSpan(traceContext.getTrace());
        span.setParentId(lastSpanId);
        trace.setLastSpanId(span.getId());
...
//    }
    ...
    @Override
    public void suspendTrace(/*Object transaction*/) {
        TraceContext traceContext = TraceContext.getContext();
        TraceDefinition suspendTrace = traceContext.getTrace();

        TraceDefinition trace = TraceUtils.newTrace();
        trace.setSuspendedTraceId(suspendTrace.getTraceId());
        traceContext.setTrace(trace);
    }

    @Override
    public void resumeTrace(/*Object transaction*/) {
        TraceContext traceContext = TraceContext.getContext();
        TraceDefinition trace = traceContext.getTrace();
        String suspendedTraceId = trace.getSuspendedTraceId();
        TraceDefinition resumeTrace = TraceContext.getTrace(suspendedTraceId);
        traceContext.setTrace(resumeTrace);

    }

    /**
     * spring 事务提交 委托给 begCommitDelegate
     *
     * @param status
     */
    @Override
    public void begCommitDelegate(DefaultTransactionStatus status) {
        ...
        Assert.notNull(lastSpanId, "LastSpanId is null");

        TransactionStatusSnapshot statusSnapshot = newTransactionStatusSnapshot(traceId, lastSpanId, lastSpanId, status);

        // 添加到快照集合
        TraceResourceManager.addSnapshot(traceId, statusSnapshot);

        SpanDefinition span = SpanUtils.getSpan(trace, lastSpanId);

        if (span != null) {

            span.setStatus(SpanDefinition.Status.BEG_COMMIT);
            if (span.getKind() == SpanDefinition.Kind.PARTICIPATOR) {
//           Integer span span.getParentId();
                // 设置 上一层 的 id
                trace.setLastSpanId(span.getParentId());
            }
        } else {
            trace.setLastSpanId(null);
        }
...
//            while (0 != commitLastSpanId) {
//
//                TransactionProcessRequest request = new TransactionProcessRequest(traceId, SpanDefinition.COORDINATOR_SPAN_ID, commitLastSpanId);
//
//                // ? :
//                TransactionProcessResponse response;
//                if (SpanUtils.hasLocalSpan(commitLastSpan)) {
//                    response = transactionProcessService.commit(request);
//                } else {
//                    response = TransactionProcessUtils.httpCommit(commitLastSpan.getLocalEndpoint(), request);
//                }
//
//
//                List<SpanDefinition> spans = trace.getSpans();
...
//        TraceDefinition trace = TraceContext.getTrace(statusSnapshot.getTraceId());
    ...
    private TransactionStatusSnapshot newTransactionStatusSnapshot(String traceId, int spanId, int order, DefaultTransactionStatus status) {
        TransactionStatusSnapshot statusSnapshot = new TransactionStatusSnapshot(status);

        statusSnapshot.setSpanId(spanId);
        statusSnapshot.setTraceId(traceId);
        statusSnapshot.setOrder(order);
        statusSnapshot.setTransactionManagerDelegate(this);

        statusSnapshot.setCurrentTransactionIsolationLevel(TransactionSynchronizationManager.getCurrentTransactionIsolationLevel());
        statusSnapshot.setCurrentTransactionName(TransactionSynchronizationManager.getCurrentTransactionName());
        statusSnapshot.setCurrentTransactionReadOnly(TransactionSynchronizationManager.isCurrentTransactionReadOnly());
        statusSnapshot.setActualTransactionActive(TransactionSynchronizationManager.isActualTransactionActive());
        statusSnapshot.setResources(new HashMap<>(TransactionSynchronizationManager.getResourceMap()));
        statusSnapshot.setSynchronizations(TransactionSynchronizationManager.getSynchronizations());
     ...
     */
    private void processForTransactionFunction(TraceDefinition trace, SpanDefinition span, TransactionProcessFunction processFunction) {
...
//            traceContext.setTrace(trace);
            ...
            Integer rollbackLastSpanId = rollbackLastSpan.getId();
...
//                boolean isLocalProcess = SpanUtils.hasLocalSpan(rollbackLastSpan);
                ...
                rollbackLastSpanId = response.getLastSpanId() - 1;
                rollbackLastSpan = SpanUtils.getSpan(trace, rollbackLastSpanId);
            }
    ...
    @Override
    public void begRollbackDelegate(DefaultTransactionStatus status) {
        ...
        SpanDefinition span = SpanUtils.getSpan(trace, lastSpanId);
        if (span != null) {
            span.setStatus(SpanDefinition.Status.BEG_ROLLBACK);
            if (span.getKind() == SpanDefinition.Kind.PARTICIPATOR) {
                trace.setLastSpanId(span.getParentId());
            }
        } else {
...
//        }));
    ...
    @Override
    public void triggerRollbackDelegate(TransactionStatusSnapshot statusSnapshot) {
